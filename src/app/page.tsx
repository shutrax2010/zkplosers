...
import { 
  ...
  ChevronDown, ChevronUp, Wallet, Copy, Dice5, ArrowLeft, Eye
} from 'lucide-react';
...
const WalletBar = ({ address, balance }: { address: string, balance: number }) => (
  <div className="w-full max-w-2xl bg-slate-900/40 backdrop-blur-md border border-white/10 p-3 rounded-2xl flex justify-between items-center mb-6">
    <div className="flex items-center gap-3">
      <div className="p-2 bg-purple-500/20 rounded-lg text-purple-400 border border-purple-500/20"><Wallet size={16} /></div>
      <div className="text-left">
        <p className="text-[8px] text-slate-500 font-black uppercase tracking-wider font-mono">ADDRESS</p>
        <p className="text-[10px] font-mono text-slate-300">{address.slice(0, 10)}...{address.slice(-6)}</p>
      </div>
    </div>
    <div className="flex items-center gap-4">
      <div className="text-right">
        <p className="text-[8px] text-slate-500 font-black uppercase tracking-wider font-mono">BALANCE</p>
        <div className="flex items-center gap-1.5">
          <span className="text-sm font-black italic text-white font-heading">{balance} YTTM</span>
          <span className="bg-purple-500/20 text-purple-400 text-[7px] font-black px-1.5 py-0.5 rounded border border-purple-500/30">🔐 SHIELDED</span>
        </div>
      </div>
    </div>
  </div>
...
export default function Home() {
  ...
  const stateRef = useRef({ 
    gameState, battlePhase, isSecret, opponentIsSecret, selectedMove, player, opponent 
  });
  stateRef.current = { gameState, battlePhase, isSecret, opponentIsSecret, selectedMove, player, opponent };
...
const calculateVP = useCallback((
  ...
  const finalizeResolution = useCallback((action: 'BATTLE' | 'FOLD', oMove: Move) => {
    ...
    if (gameMode === 'MULTI' && opponentIsSecret && opponentStateRef.current.claimedOutcome && result !== 'FOLDED') {
      ...
      if (outcome === 'PWins' || outcome === 'WIN') {
        result = 'LOSS';
        if (isSecret) { pGain = -1; oGain = 3; } else { pGain = 0; oGain = 1; }
      } else if (outcome === 'OWins' || outcome === 'LOSS') {
        result = 'WIN';
        if (isSecret) { pGain = 3; oGain = -1; } else { pGain = 1; oGain = -1; } 
      } else {
        result = 'DRAW';
        pGain = 1; oGain = 1;
      }
    ...
    setLastRoundMoves({ p: selectedMove, o: oMove });
    ...
    setPlayer(prev => {
      ...
      return { ...prev, vp: prev.vp + pGain, hand: newHand, usedMoves: [...prev.usedMoves, selectedMove!] };
    });
  ...
  }, [selectedMove, isSecret, opponentIsSecret, round, isOnChainMode, calculateVP, gameMode]);
  ...
  const processResolution = useCallback((action: 'BATTLE' | 'FOLD') => {
    ...
    if (gameMode === 'SINGLE') {
      ...
      if (isSecret) {
        if (opponentIsSecret) {
          multiplayerWS.send({ type: 'REVEAL_HIDDEN', claimedOutcome: 'Draw', proof: selectedMove.toLowerCase() });
        } else {
        ...
        multiplayerWS.send({ type: 'REVEAL_PUBLIC', cardType: selectedMove.toLowerCase() as any });
      }
  ...
  }, [selectedMove, gameMode, isSecret, opponentIsSecret, opponent.hand, finalizeResolution, calculateVP]);
      ...
      {gameState === 'ONBOARDING' && (
        <div className="flex flex-col items-center justify-center min-h-screen p-4 animate-in fade-in duration-1000">
          <div className="cyber-panel p-10 md:p-14 rounded-[2.5rem] w-full max-w-md text-center border-white/5 relative overflow-hidden">
            ...
            <h1 className="text-4xl md:text-5xl font-black mb-4 italic tracking-tighter text-white font-heading relative z-10">LOSER&apos;S GAMBIT</h1>
            ...
            <div className="space-y-4 relative z-10">
              <div className="flex items-center justify-center gap-2 mb-2">
                ...
                <span className="text-[9px] font-black tracking-widest text-slate-400 uppercase">Lace Wallet {isLaceDetected ? 'Detected' : 'Not Found'}</span>
              </div>
        ...
        <div className="flex flex-col items-center justify-center min-h-screen p-4 animate-in zoom-in duration-500">
          ...
          <div className="w-full max-w-2xl cyber-panel p-8 md:p-12 rounded-[2.5rem] border-white/5 shadow-2xl">
            ...
            <div className="flex items-center justify-between p-4 bg-black/30 rounded-2xl border border-white/5 mb-8">
              <div className="flex items-center gap-3">
                <Zap size={20} className={isOnChainMode ? "text-yellow-400" : "text-slate-600"} />
                <div>
                  <p className="text-xs font-black uppercase tracking-tighter font-heading flex items-center gap-2">ON-CHAIN MODE {isOnChainMode && <span className="text-[7px] bg-yellow-400/20 text-yellow-400 px-1.5 py-0.5 rounded font-mono border border-yellow-400/20">DEMO — simulated</span>}</p>
                  <p className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">{isOnChainMode ? "SIMULATED TRANSACTION FLOW" : "OFF-CHAIN FAST PLAY"}</p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
              <button disabled={!playerName} onClick={() => handleModeSelect('SINGLE')} className="group relative bg-slate-800/30 p-8 rounded-3xl border border-white/5 hover:border-accent-cyan/50 hover:bg-accent-cyan/5 transition-all flex flex-col items-center disabled:opacity-20 cursor-pointer">
                ...
                <p className="text-[9px] text-slate-500 font-bold uppercase tracking-[0.2em] mt-2 font-mono">VS ENFORCER AI</p>
              ...
              <button disabled={!playerName} onClick={() => handleModeSelect('MULTI')} className="group relative bg-slate-800/30 p-8 rounded-3xl border border-white/5 hover:border-accent-purple/50 hover:bg-accent-purple/5 transition-all flex flex-col items-center disabled:opacity-20 cursor-pointer">
                ...
                <p className="text-[9px] text-slate-500 font-bold uppercase tracking-[0.2em] mt-2 font-mono">P2P REAL-TIME</p>
                <span className="mt-2 bg-slate-900 text-slate-500 text-[8px] font-black px-2 py-0.5 rounded font-mono">ONLINE</span>
              </button>
            </div>
            <div className="bg-white/5 border border-white/10 p-5 rounded-2xl flex items-start gap-4">
              ...
              <div>
                <p className="text-xs font-black uppercase text-white font-heading mb-1 tracking-wider">QUICK RULES</p>
                <p className="text-[10px] text-slate-400 leading-relaxed uppercase font-mono tracking-tight">3 Rounds. Use Rock-Paper-Scissors to win VP. <span className="text-accent-purple font-bold">Hidden mode</span> grants +3 VP but costs -1 VP on loss. Winner claims <span className="text-accent-cyan font-bold italic">10 YTTM 🔐</span>.</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="w-full max-w-md">
            <div className="cyber-panel p-10 rounded-[2.5rem] text-center border-accent-cyan/30 shadow-2xl">
              <div className="flex justify-between items-center mb-10">
                <button onClick={() => setGameState('MODE_SELECTION')} className="p-2 text-slate-500 hover:text-white transition-colors cursor-pointer"><ArrowLeft size={20} /></button>
                <h2 className="text-2xl font-black italic uppercase tracking-tighter font-heading text-white">ESTABLISH CHANNEL</h2>
                <div className="w-8"></div>
              </div>
              <div className="mb-10 text-left">
                <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.3em] mb-3 ml-1 font-mono">ENTER ROOM ID</p>
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setRoomId(generateRoomId())} className="bg-slate-800 hover:bg-slate-700 p-5 rounded-2xl text-slate-400 hover:text-accent-cyan transition-colors cursor-pointer border border-white/5" title="Random ID"><Dice5 size={24} /></button>
                </div>
              </div>
          ...
          <div className="text-center w-full max-w-sm">
            ...
            {gameMode === 'MULTI' && !opponentFound && (
              <div className="cyber-panel p-6 rounded-3xl border-white/10 mb-8 animate-pulse">
                <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.3em] mb-3 font-mono">YOUR ROOM ID</p>
                <div className="flex items-center justify-center gap-4 bg-black/40 p-4 rounded-xl border border-white/5">
                  ...
                  <button onClick={() => { navigator.clipboard.writeText(roomId); }} className="p-2.5 bg-white/5 rounded-lg hover:bg-white/10 transition-colors text-slate-400 cursor-pointer"><Copy size={18} /></button>
                </div>
                <p className="text-[9px] text-slate-400 mt-4 uppercase tracking-widest font-mono">Share this code with your opponent</p>
              </div>
            )}
            <p className="text-slate-500 font-bold tracking-[0.3em] uppercase text-[9px] font-mono">ESTABLISHING SECURE CHANNEL...</p>
          </div>
          {gameMode === 'MULTI' && (
            <button onClick={() => { multiplayerWS.disconnect(); setGameState('LOBBY'); }} className="mt-12 py-3 px-8 bg-slate-800/50 hover:bg-accent-crimson/20 rounded-xl border border-white/5 text-rose-500 hover:text-accent-crimson text-[10px] font-black uppercase tracking-[0.3em] transition-all cursor-pointer font-mono">CANCEL SYNC</button>
          )}
        </div>
      )}
      ...
      {gameState === 'BATTLE' && (
        <div className="flex flex-col h-screen max-h-screen w-full max-w-4xl mx-auto overflow-hidden text-white relative py-4 px-6 md:px-10 select-none">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-black italic tracking-tighter text-white font-heading">LOSER&apos;S GAMBIT</h1>
            <div className="bg-white/5 px-6 py-2 rounded-full border border-white/10 font-black text-[10px] tracking-[0.4em] uppercase text-slate-400 font-heading">ROUND {round} / 3 {isOnChainMode && <span className="text-yellow-500 ml-2">⛓ ON-CHAIN</span>}</div>
          </div>
          ...
          <div className="flex flex-col mb-4">
            <div className="flex justify-between items-start mb-2 px-2">
              ...
              <div className="text-right">
                <span className="text-[8px] font-black text-slate-500 tracking-[0.2em] uppercase font-mono block mb-1">SCORE</span>
            ...
            <div className="flex gap-2 px-2">
              ...
              <div className="flex-1 flex items-center justify-end px-2"><p className="text-[9px] text-slate-500 italic uppercase font-mono tracking-widest">{battlePhase === 'SELECT' ? 'Thinking...' : battlePhase === 'WAITING_OPPONENT' ? 'Committed' : 'Deciding...'}</p></div>
            </div>
          </div>
          ...
          <div className="flex-1 relative flex flex-col items-center justify-center border-y border-white/5 py-4 my-2">
             <div className="flex gap-6 md:gap-12 items-center justify-center w-full transform scale-[0.85] md:scale-100">
              <div className={`w-32 h-48 md:w-40 md:h-56 rounded-[2rem] border-2 flex flex-col items-center justify-center transition-all duration-700 relative overflow-hidden ${battlePhase === 'SELECT' ? 'bg-black/20 border-white/5 opacity-10 scale-90' : opponentIsSecret ? 'bg-gradient-to-br from-[#1e1b4b] to-[#312e81] border-accent-purple/50 glow-hidden scale-110 shadow-2xl' : 'bg-slate-800/80 border-white/20 scale-110 shadow-2xl'}`}>
                {battlePhase !== 'SELECT' && ( <> <div className={`p-4 md:p-6 rounded-[1.5rem] mb-4 ${opponentIsSecret ? 'bg-accent-purple text-white shadow-lg' : 'bg-slate-700 text-white'}`}>{opponentIsSecret ? <Lock size={40} /> : <Unlock size={40} />}</div> <span className="text-[10px] font-black tracking-[0.3em] uppercase font-mono">{opponentIsSecret ? 'HIDDEN' : 'PUBLIC'}</span> </> )}
              </div>
            ...
            {roundResult && (
              <div className="absolute inset-0 z-40 flex items-center justify-center bg-background/60 backdrop-blur-md rounded-[2.5rem] animate-reveal">
                <div className="text-center px-6 py-8 cyber-panel rounded-3xl w-full max-w-sm">
                  <h2 className={`text-4xl md:text-5xl font-black italic tracking-tighter leading-none mb-4 font-heading ${roundResult === 'WIN' ? 'text-accent-cyan' : roundResult === 'LOSS' ? 'text-accent-crimson' : roundResult === 'FOLDED' ? 'text-slate-400' : 'text-accent-cyan'}`}>{roundResult === 'WIN' ? 'YOU WIN' : roundResult === 'LOSS' ? 'YOU LOSE' : roundResult === 'FOLDED' ? 'FOLD' : 'DRAW'}</h2>
                  <div className="flex gap-8 justify-center mb-8">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-[10px] font-black uppercase font-mono">YOU</span>
                    </div>
                    ...
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-[10px] font-black uppercase font-mono">OPPONENT</span>
                    </div>
                  </div>
                  <div className="flex flex-col gap-2 mb-8 bg-black/40 p-4 rounded-xl border border-white/5 font-mono text-[10px]">
                    <div className="flex justify-between items-center"><span className="text-slate-500 uppercase">You:</span><span className={`font-black ${history[history.length-1]?.playerVPGain >= 0 ? 'text-accent-cyan' : 'text-accent-crimson'}`}>{history[history.length-1]?.playerVPGain > 0 ? `+${history[history.length-1]?.playerVPGain}` : history[history.length-1]?.playerVPGain} VP</span></div>
                    <div className="flex justify-between items-center"><span className="text-slate-500 uppercase">{opponent.name}:</span><span className={`font-black ${history[history.length-1]?.opponentVPGain >= 0 ? 'text-accent-cyan' : 'text-accent-crimson'}`}>{history[history.length-1]?.opponentVPGain > 0 ? `+${history[history.length-1]?.opponentVPGain}` : history[history.length-1]?.opponentVPGain} VP</span></div>
                  </div>
                  <button onClick={handleNextRoundClick} className="w-full py-4 bg-white text-slate-900 hover:bg-slate-100 rounded-xl font-black text-sm uppercase tracking-widest font-heading transition-all cursor-pointer">
                    {round < 3 ? 'READY FOR NEXT ROUND →' : 'SEE FINAL RESULTS →'}
                  </button>
                </div>
              </div>
            )}
            ...
            {(isZkpGenerating || (battlePhase === 'WAITING_OPPONENT') || (battlePhase === 'WAITING_NEXT_ROUND')) && (
              <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-background/80 rounded-[3rem] overflow-hidden backdrop-blur-xl border border-white/5">
                ...
                <p className="text-[9px] text-slate-500 mt-2 tracking-widest uppercase font-black font-mono">{isZkpGenerating ? 'Constructing Shielded Proof...' : 'Please hold for channel synchronization'}</p>
              </div>
            )}
          </div>
      ...
      {gameState === 'RESULT' && (
        <div className="flex flex-col items-center justify-start min-h-screen p-6 md:p-10 overflow-y-auto animate-in zoom-in duration-700 bg-background relative">
          ...
          <div className="w-full max-w-4xl text-center mb-10">
            ...
            <h1 className="text-7xl md:text-[8rem] font-black italic tracking-tighter leading-none mb-6 font-heading text-white">{player.vp > opponent.vp ? 'VICTORY' : player.vp < opponent.vp ? 'DEFEAT' : 'DRAW'}{isOnChainMode && <span className="inline-block text-xl ml-4 font-mono vertical-middle align-middle opacity-50">⛓</span>}</h1>
          ...
          {player.vp > opponent.vp && (
            <div className="w-full max-w-4xl mb-12 animate-reveal"><div className="cyber-panel p-8 md:p-10 rounded-[2.5rem] border-accent-cyan/30 flex flex-col items-center justify-center relative overflow-hidden"><div className="absolute -top-12 -left-12 w-32 h-32 bg-accent-cyan/10 blur-[40px] rounded-full"></div>{!claimSuccess ? ( <> <button disabled={isClaiming} onClick={handleClaimReward} className="group w-full max-w-md py-6 bg-accent-cyan text-slate-900 rounded-2xl font-black text-xl flex items-center justify-center gap-4 shadow-[0_0_30px_rgba(6,182,212,0.3)] active:scale-[0.98] transition-all uppercase tracking-widest font-heading cursor-pointer hover:bg-cyan-300">{isClaiming ? <RefreshCw className="animate-spin" size={24} /> : <Coins size={24} />} {isClaiming ? 'PROCESSING...' : `Claim Shielded YTTM`}</button> <p className="mt-5 text-[10px] text-slate-500 font-bold uppercase tracking-[0.3em] flex items-center gap-2 font-mono"><Lock size={12} className="text-accent-purple" /> 🔐 Shielded — 第三者には非公開 (ZKP)</p> </> ) : ( <div className="text-center animate-reveal"><div className="inline-flex p-4 bg-green-500/10 rounded-full text-green-400 mb-4 border border-green-500/20"><CheckCircle size={40} /></div><h3 className="text-2xl font-black italic uppercase tracking-tighter font-heading text-white">REWARD CLAIMED</h3><p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-2 font-mono">10 YTTM を受け取りました 🔐 / 残高: {player.balance} YTTM</p></div> )}</div></div>
          )}
          <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">{history.map((h, i) => ( <div key={i} className="cyber-panel p-6 rounded-3xl border-white/5 relative overflow-hidden flex flex-col group hover:border-white/20 transition-all"><div className="flex justify-between items-center mb-6"><span className="text-[10px] font-black text-slate-500 uppercase tracking-widest font-mono">ROUND {h.round}</span><span className={`text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest font-heading ${h.result === 'WIN' ? 'bg-accent-cyan/10 text-accent-cyan border border-accent-cyan/20' : h.result === 'LOSS' ? 'bg-accent-crimson/10 text-accent-crimson border border-accent-crimson/20' : h.result === 'FOLDED' ? 'bg-slate-800 text-slate-400 border border-white/5' : 'bg-white/5 text-white border border-white/10'}`}>{h.result}</span></div><div className="flex justify-between items-center mb-6 px-2"><div className="flex flex-col items-center gap-2"><div className={`p-3 rounded-xl border ${h.playerIsSecret ? 'bg-accent-purple/10 border-accent-purple/30 text-accent-purple' : 'bg-accent-cyan/5 border-accent-cyan/20 text-accent-cyan'}`}>{h.playerIsSecret ? <Lock size={24} /> : <MoveIcon move={h.playerMove} size={24} />}</div><span className="text-[8px] font-black uppercase text-slate-500 font-mono tracking-tighter">{h.playerIsSecret ? 'HIDDEN' : h.playerMove}</span></div><span className="text-lg font-black text-white/10 font-heading italic">VS</span><div className="flex flex-col items-center gap-2"><div className={`p-3 rounded-xl border ${h.opponentIsSecret ? 'bg-accent-purple/10 border-accent-purple/30 text-accent-purple' : 'bg-accent-crimson/5 border-accent-crimson/20 text-accent-crimson'}`}>{h.opponentIsSecret ? <Lock size={24} /> : <MoveIcon move={h.opponentMove} size={24} />}</div><span className="text-[8px] font-black uppercase text-slate-500 font-mono tracking-tighter">{h.opponentIsSecret ? 'HIDDEN' : h.opponentMove}</span></div></div><div className="mt-auto pt-6 border-t border-white/5 flex justify-between items-center font-mono text-[9px] font-black tracking-wider uppercase"><span className={h.playerVPGain > 0 ? 'text-accent-cyan' : h.playerVPGain < 0 ? 'text-accent-crimson' : 'text-slate-500'}>You {h.playerVPGain > 0 ? `+${h.playerVPGain}` : h.playerVPGain} VP</span><span className={h.opponentVPGain > 0 ? 'text-accent-cyan' : h.opponentVPGain < 0 ? 'text-accent-crimson' : 'text-slate-500'}>{opponent.name} {h.opponentVPGain > 0 ? `+${h.opponentVPGain}` : h.opponentVPGain} VP</span></div></div> ))}</div>
          {isOnChainMode && (
            <div className="w-full max-w-4xl cyber-panel rounded-[2.5rem] border-yellow-500/20 overflow-hidden mb-12">
              <button onClick={() => setShowOnChainPanel(!showOnChainPanel)} className="w-full p-8 flex justify-between items-center hover:bg-white/5 transition-colors cursor-pointer"><div className="flex items-center gap-4 text-yellow-500 font-heading"><Zap size={24} /><h3 className="text-xl font-black italic uppercase tracking-tighter">ON-CHAIN DATA LOG</h3></div>{showOnChainPanel ? <ChevronUp /> : <ChevronDown />}</button>
              {showOnChainPanel && ( <div className="p-8 pt-0 animate-in slide-in-from-top-4"><div className="space-y-4 font-mono text-[10px] bg-black/40 p-6 rounded-2xl border border-white/5"><div className="flex justify-between border-b border-white/5 pb-2"><span className="text-slate-500 uppercase">Contract Address</span><span className="text-yellow-400 underline italic">midnight_contract_92x7...a290</span></div><div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4"><div className="space-y-3"><p className="text-accent-cyan font-black border-b border-accent-cyan/20 pb-1 uppercase tracking-widest">YOUR STREAM</p>{history.map((h, i) => ( <div key={i} className="flex flex-col gap-1"><span className="text-slate-500 text-[8px] uppercase tracking-tighter">Round {h.round} Deployment</span><span className="text-slate-300 truncate block bg-white/5 p-1.5 rounded">{h.txHash}</span></div> ))}</div>{gameMode === 'MULTI' && ( <div className="space-y-3"><p className="text-accent-crimson font-black border-b border-accent-crimson/20 pb-1 uppercase tracking-widest">OPPONENT STREAM</p>{opponentTxHashes.map((hash, i) => ( <div key={i} className="flex flex-col gap-1"><span className="text-slate-500 text-[8px] uppercase tracking-tighter">Peer Verification TX</span><span className="text-slate-300 truncate block bg-white/5 p-1.5 rounded">{hash}</span></div> ))}</div> )}</div><div className="mt-8 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl text-yellow-500/70 italic text-center font-mono text-[9px] tracking-wide uppercase">DEMO: Midnight Preprod Simulated Environment. Real ZKP proofs generated locally.</div></div></div> )}
            </div>
          )}
          <button onClick={() => { setGameState('MODE_SELECTION'); setHistory([]); setOpponentTxHashes([]); setRound(1); setClaimSuccess(false); if (gameMode === 'MULTI') multiplayerWS.disconnect(); }} className="w-full max-w-md py-6 text-slate-500 hover:text-white transition-all underline underline-offset-8 text-[10px] font-black tracking-[0.4em] uppercase mb-20 cursor-pointer font-heading">← Return to Lobby</button>
        </div>
      )}
    </main>
  );
