...
### 2.1 ログイン・ロビー画面 (Lobby Screen)
対戦の入り口となる画面。

- **ヘッダー (Wallet Bar):**
  - ゲームロゴ（左側）。
  - ウォレット接続ボタン（右側）。接続後は短縮アドレスとYTTMトークン残高を表示。
- **メインパネル (Matchmaking):**
  - **Room ID 入力:** 英数字のテキストフィールド。
  - **Enter Room ボタン:** 入力されたIDでルームに参加・作成。
- **ステータス表示:**
  - `Waiting for opponent...`: 相手を待っている状態。
  - `Match Found!`: 相手が見つかり、ゲームボードへ遷移する直前の演出。
- **ルールサマリー:** 初心者向けにVP表と「負け犬」カードの性質を簡潔に表示。
