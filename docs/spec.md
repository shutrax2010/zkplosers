...
フロントエンド（Next.js）とバックエンド（WebSocket リレー）を **1つの Web Service** で運用。

Express が Next.js ビルド成果物（`.next`）またはスタンドアロン出力を静的配信し、WebSocket を同一ポートで処理する。
※簡略化のため、Render 上では `npm run build` 後に `node server/dist/index.js` で起動する構成。

```
https://zkplosers.onrender.com
  ├── HTTP  → Express → Static Assets & Next.js
...
| `NEXT_PUBLIC_WS_URL` | `wss://zkplosers.onrender.com` | フロントエンドの WS 接続先（ビルド時に埋め込み） |

> **注意:** `NEXT_PUBLIC_WS_URL` はビルド時に静的に埋め込まれる。変更後は再デプロイが必要。
...
ローカルでは `NEXT_PUBLIC_WS_URL` を設定しない場合、`ws://localhost:3001` にフォールバック。
  ...
  - type: web
    ...
    envVars:
      ...
      - key: NEXT_PUBLIC_WS_URL
        value: wss://zkplosers.onrender.com
    plan: free
```
